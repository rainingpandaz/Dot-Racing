...
public class DotRaceArrayList extends JPanel {
  ...
  int trackLength = 300;
  int startLine = 50;
  // number of milliseconds (one thousandth of a second) to pause between each cycle
  int pauseTime = 15;
  ...
  public void initializeRacers() {
    int racerCount = (int)(Math.random()*5) + 3; // play around with this, trying different numbers.
...
dots.add(aDotObject);
  ...
  private Dot whoMoves() {
    // finish this by returning one of the dots in the list chosen randomly
  ...
  public Dot whoseAhead(){
    int currentLeadDistance = 0;
    for (Dot aDotObject : dots){
      if (aDotObject.getDistance() > currentLeadDistance){
        currentLeader = aDotObject;
        currentLeadDistance = currentLeader.getDistance();
      }
    }
  ...
  public static void main(String[] args) {
    DotRaceArrayList dotlist = new DotRaceArrayList();
